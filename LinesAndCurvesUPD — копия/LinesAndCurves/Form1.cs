...
namespace LinesAndCurves
{
    ...
    {
        private List<IDrawable> ListOfCurves;

        public Form1()
        {
            InitializeComponent();
            ListOfCurves = new List<IDrawable>();
        }
        ...
        private void Generate_button_Click(object sender, EventArgs e)
        {
            ...
            ICurve lineCurve = new Line(a, b);
            AVisualCurve L = new AVisualCurve(lineCurve, Color.Black);

            ICurve bezierCurve = new Bezier(a, b, c, d);
            AVisualCurve B = new AVisualCurve(bezierCurve, Color.Green);

            MoveTo moveTo = new MoveTo(new Fragment(bezierCurve, 0.66, 0.33), new Geometry.Point(300, 300));
            ICurve curveFromMoveTo = moveTo.GetCurve();
            AVisualCurve mfB = new AVisualCurve(curveFromMoveTo, Color.Black);
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                black = new VerticalReflection(new HorizontalReflection(new BlackDrawer(e.Graphics, Color.Black), panel1.Width), panel1.Height);
                green = new VerticalReflection(new HorizontalReflection(new GreenDrawer(e.Graphics, Color.Green), panel1.Width), panel1.Height);
            }
            else if (VReflection_checkbox.Checked)
            {
                black = new VerticalReflection(new BlackDrawer(e.Graphics, Color.Black), panel1.Height);
                green = new VerticalReflection(new GreenDrawer(e.Graphics, Color.Green), panel1.Height);
            }
            else if (HReflection_checkbox.Checked)
            {
                black = new HorizontalReflection(new BlackDrawer(e.Graphics, Color.Black), panel1.Width);
                green = new HorizontalReflection(new GreenDrawer(e.Graphics, Color.Green), panel1.Width);
            }
            else
            {
                black = new BlackDrawer(e.Graphics, Color.Black);
                green = new GreenDrawer(e.Graphics, Color.Green);
            }
        ...
        private void Save_button_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                using (StreamWriter writer = new StreamWriter(filePath))
                {
                    ...
                    //foreach (var curve in ListOfCurves)
                    //{
                    //    curve.Draw(new SVGExporter(), writer);
                    //}
